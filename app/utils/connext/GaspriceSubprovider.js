...
export default class GaspriceSubprovider extends Subprovider {
  ...
  handleRequest(payload: any, next: () => void, end: (err: any, res?: any) => void) {
    ...
    this.estimateGasPriceFromHub()
      .catch(err => {
        console.warn(
          'Error fetching gas price from the hub (falling back to Web3):',
          err
        );
        return null;
      })
  ...
  async estimateGasPriceFromHub(): Promise<BigNumber | null> {
  ...
  estimateGasPriceFromPreviousBlocks(): Promise<BigNumber> {
  ...
  getBlock(item: string): Promise<Transaction[]> {
    return new Promise((resolve, reject) =>
      this.emitPayload(
        { method: 'eth_getBlockByNumber', params: [item, true] },
        (err: any, res: any) => {
          if (err) {
            return reject(err);
          }
  ...
  meanGasPrice(blocks: Transaction[][]): BigNumber {
