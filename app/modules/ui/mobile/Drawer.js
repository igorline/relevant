...
export default class DrawerView extends React.PureComponent<Props> {
  ...
  isDrawerTypeFront =
    new Value() < Binary > (this.props.drawerType === 'front' ? TRUE : FALSE);

  isLocked = new Value(this.props.locked ? TRUE : FALSE);

  isOpen = new Value() < Binary > (this.props.open ? TRUE : FALSE);

  nextIsOpen = (new Value() < Binary) | (UNSET < -1);

  isSwiping = new Value() < Binary > FALSE;

  gestureState = new Value() < number > State.UNDETERMINED;

  touchX = new Value() < number > 0;

  velocityX = new Value() < number > 0;

  gestureX = new Value() < number > 0;

  offsetX = new Value() < number > 0;

  position = new Value() < number > 0;

  containerWidth = new Value() < number > 0;

  drawerWidth = new Value() < number > 0;

  drawerOpacity = new Value() < number > 0;
  ...
  swipeVelocityThreshold = new Value() < number > this.props.swipeVelocityThreshold;

  currentOpenValue: boolean = this.props.open;

  pendingOpenValue: boolean | undefined;

  isStatusBarHidden: boolean = false;

  manuallyTriggerSpring = new Value() < Binary > FALSE;

  transitionTo = (isOpen: number | Animated.Node<number>) => {
  ...
  handleDrawerLayout = (e: LayoutChangeEvent) => {
  ...
  toggleStatusBar = (hidden: boolean) => {
    ...
    return (
      <DrawerProgressContext.Provider value={this.progress}>
        <PanGestureHandler
          ...
          {...gestureHandlerProps}
        >
          <Animated.View onLayout={this.handleContainerLayout} style={styles.main}>
            <Animated.View
              style={[
                styles.content,
                {
